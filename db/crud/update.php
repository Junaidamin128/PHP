...
if (isset($_POST["form"])) {
    ...
    switch ($form) {
        ...
        case "folder-delete":
            $id = $_POST['id'];
            $query = $conn->query("DELETE FROM folder WHERE folderID=$id");
            if ($query) {
                $msgs['success'][] = "Delete success";
            } else {
                $msgs['danger'][] = "Delete failed";
            }
            break;
        case "file-delete":
            $id = $_POST['id'];
            $query = $conn->query("DELETE FROM file WHERE fileID=$id");
            if ($query) {
                $msgs['success'][] = "Delete success";
            } else {
                $msgs['danger'][] = "Delete failed";
        ...
        case "folder-update":
            $id = $_POST['id'];
            $name = $_POST['name'];
            $query = $conn->query("UPDATE folder SET name='$name' WHERE folderID=$id");
            if ($query) {
                $msgs['success'][] = "Update success";
            } else {
                $msgs['danger'][] = "Update failed";
            }
            break;
            case "file-update":
                $id = $_POST['id'];
                $name = $_POST['name'];
                $fid = $_POST['folder'];
                $query = $conn->query("UPDATE `file` SET `name` = '$name', `fid` = '$fid' WHERE `fileID` = $id");
                if ($query) {
                    $msgs['success'][] = "Update success";
                } else {
                    $msgs['danger'][] = "Update failed";
                    ...
                    <tbody>
                        ...
                        ?>
                            <tr>
                                ...
                                <td>
                                    <form method="post">
                                        <input type="hidden" name="form" value="file-delete" />
                                        <input type="hidden" name="id" value="<?= $row['fileID']; ?>" />
                                        <button>Delete</button>
                                    </form>
                                    <form method="POST">
                                        <input type="hidden" name="form" value="file-update" />
                                        <input type="hidden" name="id" value="<?= $row['fileID']; ?>" />
                                        <label>
                                            Name:
                                        ...
                                        <label> Folder
                                            <select name="folder">
                                                <option value="">--Select--</option>
                                                <?php foreach ($folders as $id => $folder) { ?>
                                                    <option <?= $id==$row['fid']?"selected": ""?> value="<?= $id ?>"><?= $folder['name'] ?></option>
                                                <?php } ?>
                                            </select>
                    ...
                    <tbody>
                        ...
                        ?>
                            <tr>
                                ...
                                <td>
                                    <form method="post">
                                        <input type="hidden" name="form" value="folder-delete" />
                                        <input type="hidden" name="id" value="<?= $folder['folderID']; ?>" />
                                        <button>Delete</button>
                                    </form>

                                    <form method="post">
                                        <input type="hidden" name="form" value="folder-update" />
                                        <input type="hidden" name="id" value="<?= $folder['folderID']; ?>" />
                                        <input name="name" value="<?= $folder['name'] ?>" />
                                        <button>Update</button>
                                    </form>
